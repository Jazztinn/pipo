...
test("website includes canonical Pipo MenuWeb bundle", async () => {
  ...
  assert.match(html, /id="pipo-header"[^>]*h-10/);
  assert.match(html, /id="header-title"[^>]*absolute inset-0 flex items-center justify-center/);
  assert.match(html, /id="sync-button"[^>]*absolute right-4[^>]*max-w-\[108px\]/);
  assert.match(html, /html\[data-host-mode="menuBar"\] #main-panel\.mac-window,[\s\S]*#inspector-panel\.mac-window[\s\S]*backdrop-filter: none/);
